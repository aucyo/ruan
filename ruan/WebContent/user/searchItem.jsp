...
function setSearchType(t){
	...
	<table class="table table-hover">
  	...
  	<thead>
  		...
  		<td style="font-weight: bold;">发布人</td>
  		<td style="font-weight: bold;">发布报价</td>
  		<td style="font-weight: bold;">发布日期</td>
  		<td style="font-weight: bold;">所属类别</td>
  	</thead>
  	<s:iterator value="searchItems" var="item">
	    <tr>
	      <td><a href="hisItem?myBasic.uid=${myBasic.uid}&hisBasic.uid=${item.uid}&hisItem.iid=${item.iid}">${item.iname}</a></td>
	      <td><a href="hisInform?myBasic.uid=${myBasic.uid}&hisBasic.uid=${item.uid}">${item.sname}</a></td>
	      <td>${item.cprice}</td>
	      <td>${item.date}</td>
	      <td>${item.category}</td>
	    </tr>
